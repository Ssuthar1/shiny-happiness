...
class DestinationCategoryManagement extends Component
{
    ...
    public  $destination_category_id,$title,$meta_title,$meta_tag_keywords,$meta_tag_descriptions,$short_description, $description,$status;
    public $module_title='destination_category',$buttonText='';
    ...
    {         
        $data  = DestinationCategory::where('title', 'like', '%'.$this->s.'%')->orWhere('description', 'like', '%'.$this->s.'%');
        $data = $data->orderby('id','DESC')->paginate(10);
        $this->isloading=false;
        return view('livewire.dashboard.destination-category.index',compact('data'));
    }
    ...
    public function store()
    {
        ...
        $dataInfo = DestinationCategory::create($validatedData);
        $id = $dataInfo->id;
        if(!empty($validatedData['photo']))
        {
           $this->saveImage($validatedData['title'],$validatedData['photo'],'destination_category',$id);             
        }
        ...
        if(!empty($validatedData['gallery']))
        {
            ...
            {
                $this->saveImage($validatedData['title'],$galleryImage,'destination_category',$id,'gallery_image');
            }
        }
    ...
    {
        $editInfo = DestinationCategory::findOrFail($id);
        $this->destination_category_id = $id;
     ...
     public function update()
    {
        ...
        $data = DestinationCategory::find($this->destination_category_id);
        $data->update([
            'title' => $this->title,
         ...
         $id = $this->destination_category_id;
        if(!empty($validatedData['photo']))
        {
   ...
   public function changeStatus($id){
        $status = DestinationCategory::find($id);
        // dd($status->status);
        if ($status['status'] == '1') {
    ...
    {
        DestinationCategory::find($id)->delete();
