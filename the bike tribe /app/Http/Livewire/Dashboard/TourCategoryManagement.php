...
class TourCategoryManagement extends Component
{
    ...
    public  $tour_category_id,$title,$meta_title,$meta_tag_keywords,$meta_tag_descriptions,$short_description, $description,$status;
    public $module_title='tour_category',$buttonText='';
    ...
    {         
        $data  = TourCategory::where('title', 'like', '%'.$this->s.'%')->orWhere('description', 'like', '%'.$this->s.'%');
        $data = $data->orderby('id','DESC')->paginate(10);
        $this->isloading=false;
        return view('livewire.dashboard.tour-category.index',compact('data'));
    }
    ...
    public function store()
    {
        ...
        $dataInfo = TourCategory::create($data);
        $id = $dataInfo->id;
        if(!empty($validatedData['photo']))
        {
           $this->saveImage($validatedData['title'],$validatedData['photo'],'tour_category',$id); 
        }
        ...
        if(!empty($validatedData['gallery']))
        {
            ...
            {
                $this->saveImage($validatedData['title'],$galleryImage,'tour_category',$id,'gallery_image');
            }
        } 
         
        $this->dispatchBrowserEvent('successMessage', [ 
                    'message' => __("Tour Category Created Successfully."), 
                ]);
    ...
    {
        $editInfo = TourCategory::findOrFail($id);
        $this->tour_category_id = $id;
     ...
     public function update()
    {
        ...
        $data = TourCategory::find($this->tour_category_id);
        $data->update([
            'title' => $this->title,
        ...
        $this->dispatchBrowserEvent('successMessage', [ 
                    'message' => __("Tour Category Updated Successfully."), 
                ]);
        $this->resetInputFields();
    }

   public function changeStatus($id){
        $status = TourCategory::find($id);
        // dd($status->status);
        if ($status['status'] == '1') {
    ...
    public function delete($id)
    {
        ...
        $this->dispatchBrowserEvent('successMessage', [ 
                    'message' => __("Tour Category Deleted Successfully."), 
                ]);
    }
