  ...
  @else  <div class="p-7"> 
    <div class="mb-5.5 flex flex-col gap-5.5 sm:flex-row">
      ...
      <div class="col-md-4">
        <div class="form-group">
          <label> Search :&nbsp; </label>  
          <input type="text" class="w-full rounded border border-stroke bg-gray py-3 pl-11.5 pr-4.5 font-medium text-black focus:border-primary focus-visible:outline-none dark:border-strokedark dark:bg-meta-4 dark:text-white dark:focus:border-primary" placeholder="Search by booking id" wire:model.live="s">
        </div>
     </div>
   </div>
          ...
          @php $i = 1; @endphp
            @foreach($data as $depKey => $bookingDetail)
              <tr>
                ...
                <td class="border-b border-[#eee] py-5 px-4 pl-9 dark:border-strokedark">
                  <p class="text-sm">{{$bookingDetail->name ?? ''}}</p>
                </td>
                <td class="border-b border-[#eee] py-5 px-4 pl-9 dark:border-strokedark">
                  <p class="text-sm">{{$bookingDetail->mobile_no ?? ''}}</p>
                </td>
                <td class="border-b border-[#eee] py-5 px-4 pl-9 dark:border-strokedark">
                  <p class="text-sm">{{$bookingDetail->email ?? ''}}</p>
                </td>
                <td class="border-b border-[#eee] py-5 px-4 pl-9 dark:border-strokedark">
                ...
                <td class="border-b border-[#eee] py-5 px-4 dark:border-strokedark">
                  <div class="flex items-center space-x-3.5">
                    ...
                    @endcan
                    @can('booking-delete')
                    <button class="hover:text-primary"  wire:click="$dispatch('triggerDetail',{{$bookingDetail->id }})"
>
