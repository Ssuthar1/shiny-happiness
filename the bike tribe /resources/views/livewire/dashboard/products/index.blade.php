<div>
  @if(isset($pageType) && $pageType == 'edit')
    @include('livewire.dashboard.products.create')
  @elseif(isset($pageType) && $pageType == 'addInventory')
    @include('livewire.dashboard.products.create-inventory')
  @elseif(isset($pageType) && $pageType == 'inventoryIndex')
    @include('livewire.dashboard.products.inventory-index')
  @else  
  <div class="p-7"> 
    @can('add-products')
    <div class="flex justify-end gap-4.5"> 
      <button
        ...
        type="button" wire:click="addProduct()">
        Add Product
      </button>
    </div> 
    @endcan
            ...
            @foreach($data as $depKey => $dataDetail)
              <tr>
                ...
                <td class="border-b border-[#eee] py-5 px-4 dark:border-strokedark">
                  <p class="text-black dark:text-white">@if(isset($dataDetail->sku_id) && !empty($dataDetail->sku_id)) {{$dataDetail->sku_id ?? ''}} @endif</p>
                </td>
                <td class="border-b border-[#eee] py-5 px-4 dark:border-strokedark">
                ...
                <td class="border-b border-[#eee] py-5 px-4 dark:border-strokedark">
                  <p class="text-black dark:text-white">@if(isset($dataDetail->final_sale_price )) {{number_format($dataDetail->final_sale_price,2) }} @endif</p>
                </td>
                <td class="border-b border-[#eee] py-5 px-4 pl-9 dark:border-strokedark">
                ...
                {{-- <td class="border-b border-[#eee] py-5 px-4 pl-9 dark:border-strokedark">
                  <button class="flex justify-center rounded bg-primary py-2 px-4 font-medium text-gray hover:bg-opacity-90 mb-5" type="button" @if(isset($dataDetail->getProductInventory) && count($dataDetail->getProductInventory)) wire:click="inventoryIndex({{$dataDetail->id}})" @else wire:click="addInventory({{$dataDetail->id}})" @endif> 
                    <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-patch-plus h-6 w-6" viewBox="0 0 16 16">
...
<script src="https://cdnjs.cloudflare.com/ajax/libs/sweetalert/2.1.2/sweetalert.min.js" integrity="sha512-AA1Bzp5Q0K1KanKKmvN/4d3IRKVlv9PYgwFPvm32nPO6QS8yH1HO7LbgB1pgiOxPtfeg5zEn2ba64MUcqJx6CA==" crossorigin="anonymous" referrerpolicy="no-referrer"></script>
  <script type="text/javascript">
    ...
    document.addEventListener('DOMContentLoaded', function (e) {
        @this.on('triggerINventoryDetail', id => {
            Swal.fire({
                title: "Are you sure?",
                text: "",
                icon: "warning",
